...
document.addEventListener("DOMContentLoaded", () => {
  ...
  container.querySelectorAll(".playlist-tile:not(.new-playlist-tile)").forEach((el) => el.remove());
  ...
  addForm.addEventListener("submit", (e) => {
    ...
    const songPairs = Array.from(songsContainer.querySelectorAll(".song-pair"));
    const songs = songPairs.map((pair) => {
      const title = pair.querySelector('input[name="songTitle"]').value.trim();
      const artist = pair.querySelector('input[name="songArtist"]').value.trim();
...
function openModal(id) {
  ...
  headingWrapper.append(songListTitle, editBtn)
  headingWrapper.append(songListTitle, deleteBtn)
